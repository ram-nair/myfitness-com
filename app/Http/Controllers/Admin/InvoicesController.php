...
class InvoicesController extends BaseController
{
    ...
    public function index()
    {
        ...
        $this->setPageTitle('invoices', 'invoices List');
    ...
    public function datatables()
    {
        ...
        return Datatables::of($invoices)
            ...
            })->editColumn('actions', function ($invoices) use ($currentUser) {
                $b = '';
                $b .= '<a href="' . URL::route('admin.invoices.show', $invoices->id) . '" class="btn btn-outline-primary btn-xs"><i class="fa fa-eye"></i></a>';
                return $b;
            })->make(true);
    }
    ...
    public function show(Order $order)
    {
        ...
        return view('admin.invoices.show', compact('order', 'store', 'user', 'address', 'stockProducts', 'outOfStockProducts'));
    }
    ...
    public function updateStatus(Request $request, Order $order)
    {
        ...
        if ($totalDuration < $cancelDuration) {
            ...
            return redirect()->route('admin.invoices.show', $order->id);
        }
        $prevStatus = $order->order_status;
        $order->order_status = $request->order_status;
        $order->save();
        $order->invoicestatusHistory()->save(new invoicestatus(['status' =>  $request->order_status]));
        if ($request->order_status === "out_for_delivery" && $order->payment_type === "credit_card") {
            ...
            if (isset($paymentSuccess['error'])) {
                ...
                $order->save();
                $order->invoicestatusHistory()->save(new invoicestatus(['status' =>  $prevStatus]));
        ...
        $this->createinvoicestatusPush($order->user, 300, $order, "ECOM");
        alert()->success('Invoices status updated', 'Updated');
        return redirect()->route('admin.invoices.show', $order->id);
    }
    ...
    public function updateItems(Request $request, Order $order)
    {
        ...
        $order = calculateECommerceinvoicesum($order);
